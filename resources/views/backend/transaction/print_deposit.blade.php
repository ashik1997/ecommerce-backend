...
<head>
    <meta charset="utf-8">
    <title>Deposit Receipt - #{{ $deposit->id }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
    ...
    <div class="receipt-container">
        ...
        <div class="receipt-details">
            <div class="detail-row">
                ...
                <div class="detail-value"><strong>#{{ str_pad($deposit->id, 6, '0', STR_PAD_LEFT) }}</strong></div>
            </div>
            <div class="detail-row">
                <div class="detail-label">Date:</div>
                <div class="detail-value">{{ \Carbon\Carbon::parse($deposit->deposit_date)->format('d F Y') }}</div>
            </div>
            <div class="detail-row">
                <div class="detail-label">Reference No:</div>
                <div class="detail-value">{{ $deposit->reference_no ?? 'N/A' }}</div>
            </div>
            <div class="detail-row">
                <div class="detail-label">Investor Name:</div>
                <div class="detail-value"><strong>{{ $deposit->investor->name ?? 'Owner' }}</strong></div>
            </div>
            @if($deposit->investor && $deposit->investor->phone)
            <div class="detail-row">
                <div class="detail-label">Phone:</div>
                <div class="detail-value">{{ $deposit->investor->phone }}</div>
            </div>
            @endif
            @if($deposit->investor && $deposit->investor->email)
            <div class="detail-row">
                <div class="detail-label">Email:</div>
                <div class="detail-value">{{ $deposit->investor->email }}</div>
            </div>
            @endif
            <div class="detail-row">
                <div class="detail-label">Payment Method:</div>
                <div class="detail-value"><strong>{{ $deposit->paymentType->payment_type ?? 'N/A' }}</strong></div>
            </div>
        </div>
        ...
        <div class="amount-section">
            <div class="amount-row">
                ...
                <div class="amount-value">৳ {{ number_format($deposit->amount, 2) }}</div>
            </div>
            <div class="amount-in-words">
                <strong>In Words:</strong> {{ ucwords(numberToWords($deposit->amount)) }} Taka Only
            </div>
        </div>
        ...
        <div class="footer">
            ...
            @if($deposit->creator_info)
            <p>Created by: {{ $deposit->creator_info->name }}</p>
            @endif
        </div>
    </div>
