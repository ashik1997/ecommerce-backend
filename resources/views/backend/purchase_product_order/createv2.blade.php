...
@section('header_css')
    ...
    <script src="{{ versioned_asset('assets/js/purchase_vue_v2.js') }}" defer></script>
    ...
    <style>
       ...
       ═══════════════════════════════════════════════ */
        ...
        /* Modal variant re-select */
        .po_component .po-modal-variant-strip {
            display: flex;
            align-items: center;
            justify-content: space-between;
            padding: 8px 12px;
        ...
        .po_component .po-modal-variant-pills {
            display: flex;
            flex-wrap: wrap;
            gap: 4px;
        }

        .po_component .po-modal-variant-pill {
            background: var(--t100);
            color: var(--t800);
            padding: 2px 8px;
            border-radius: 20px;
        ...
        .po_component .po-modal-change-btn {
            display: inline-flex;
            align-items: center;
            gap: 4px;
            padding: 4px 10px;
            border-radius: var(--r);
            border: 1.5px solid var(--t300, var(--t200));
            background: #fff;
            color: var(--t700);
            font-size: 11px;
            font-weight: 700;
        ...
        .po_component .po-modal-change-btn:hover {
            background: var(--t100);
        }

        /* Modal inline variant selector */
        .po_component .po-modal-attr-grid {
            display: grid;
    ...
    <div class="po_component" id="formApp">
        ...
        <div class="po-header">
            <div class="po-header-left">
                <div class="po-header-icon"><i class="fas fa-shopping-cart"></i></div>
                <div>
                    <h4>Purchase Order</h4>
                    <span>Create a new product purchase order</span>
                </div>
            </div>
        ...
        <form action="{{ url('save/new/purchase-product/order') }}" method="POST" id="purchaseForm"
            ...
            <div class="po-card">
                ...
                <div class="po-card-body">
                    <div class="po-grid-5">
                        <div class="po-fg">
                            <label class="po-label"><i class="fas fa-tag"></i> Status <span class="req">*</span></label>
                            <select name="order_status" class="po-select" required>
                                <option value="pending" selected>Draft</option>
                                <option value="received">Received</option>
                            </select>
                        </div>
                        <div class="po-fg">
                            <label class="po-label"><i class="fas fa-warehouse"></i> Warehouse <span
                            ...
                            <div class="d-flex align-items-center" style="gap: 8px;">
                                <select name="purchase_product_warehouse_id" class="po-select" required
                                    ...
                                    <option v-for="warehouse in activeWarehouseItems" :key="warehouse.id"
                                        :value="warehouse.id">
                                        @{{ warehouse.title }}
                                    </option>
                                </select>
                                <button type="button" class="btn btn-sm btn-primary" title="Manage Warehouse"
                        ...
                        <div class="po-fg">
                            <label class="po-label"><i class="fas fa-truck"></i> Supplier <span
                                    class="req">*</span></label>
                        ...
                        <div class="po-fg">
                            <label class="po-label"><i class="fas fa-calendar"></i> Purchase Date <span
                                    class="req">*</span></label>
                            <input type="date" name="purchase_date" class="po-input" value="{{ now()->format('Y-m-d') }}"
                                required>
                        </div>
                        <div class="po-fg">
                            <label class="po-label"><i class="fas fa-hashtag"></i> Reference</label>
                            <input type="text" name="reference" class="po-input" placeholder="Optional reference">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="po-card">
                <div class="po-card-header">
                    <div class="po-card-header-left">
                        <i class="fas fa-search"></i> Add Product
                    </div>
                </div>
                <div class="po-card-body">
                    ...
                    <!-- Variant selection panel (shows when product has variants) -->
                    <div v-if="pendingProduct && pendingProduct.has_variants" class="po-variant-panel">
                        <div class="po-variant-title">
                            <i class="fas fa-th-list"></i>
                            Select Variant — <span
                                style="font-weight:400;text-transform:none;letter-spacing:0;">@{{ pendingProduct.name }}</span>
                        </div>
                        <div class="po-hint" style="margin-bottom:10px;">
                            <i class="fas fa-info-circle"></i>
                            Add one variant, then select the next variant from the same product. Search another product or reset to clear this block.
                        </div>
                        ...
                        <div class="po-attr-grid">
                            <div v-for="attr in getPendingVariantAttributes()" :key="attr" class="po-attr-fg">
                                ...
                                <select class="po-attr-select" v-model="pendingVariantSelections[attr]"
                                    ...
                                    <option v-for="val in pendingProduct.product_variants[attr]" :key="val"
                                        :value="val">
                                        @{{ val }}
                                    </option>
                                </select>
                        ...
                        <div v-if="getPendingCombinationLabel()">
                            <div v-if="pendingMatchedVariant" class="po-comb-strip">
                                <span class="po-comb-label"><i class="fas fa-check-circle" style="color:var(--ok);"></i>
                                    Match:</span>
                                <span v-for="(val, key) in pendingMatchedVariant" :key="key">
                                    <span v-if="key !== 'id'" class="po-comb-pill">@{{ key }}:
                                        @{{ val }}</span>
                                </span>
                            </div>
                        ...
                        <button type="button" class="po-add-btn" :disabled="!pendingMatchedVariant"
                            @click="addProductToTable">
                            <i class="fas fa-plus"></i> Add to Order
                        </button>
                        <button type="button" class="po-btn po-btn-ghost" style="margin-left:8px;padding:8px 14px;"
                            @click="clearPendingProduct">
                    ...
                    <div v-if="pendingProduct && !pendingProduct.has_variants" style="margin-top:12px;">
                        <div class="po-comb-strip" style="display:inline-flex;">
                            ...
                            <span class="po-comb-pill" style="background:var(--g100);color:var(--g700);">Stock:
                                @{{ pendingProduct.stock }}</span>
                        </div>
                ...
                <div class="po-card-body" style="padding:0;">
                    <div class="po-table-wrap">
                        <table class="po-table">
                            ...
                            <tbody>
                                <tr v-if="purchaseItems.length === 0" class="po-empty-row">
                                    <td colspan="12">
                                ...
                                <tr v-for="(item, index) in purchaseItems" :key="item.rowKey">
                                    ...
                                    <td>
                                        <select class="po-input po-select" style="min-width:100px;"
                                            ...
                                            <option v-for="c in item.cartoonOptions" :value="c.id"
                                                :key="`c-${c.id}`">@{{ c.title }}</option>
                                        </select>
                                    </td>
                                    ...
                                    <td>
                                        <input type="number" class="po-input" style="width:68px;"
                                            ...
                                            @input="handleItemNumericInput(item, 'quantity', 'Quantity')" min="0"
                                            step="1">
                                    </td>
                                    <td>
            ...
            <div class="po-card">
                <div class="po-card-body">
                    <div class="po-bottom-grid">
                        ...
                        <div>
                            ...
                            <!-- Hidden other charges fields (kept for backend compatibility) -->
                            @foreach ($other_charges_types as $key => $item)
                                <div style="display:none;">
                                    ...
                                    <select class="other_charges_type other_charges_type{{ $key }}"
                                        ...
                                        <option value="percent" {{ $item->type == 'percent' ? 'selected' : '' }}>Per%
                                        </option>
                                        <option value="fixed" {{ $item->type == 'fixed' ? 'selected' : '' }}>Fixed
                                        </option>
                                    </select>
                                </div>
                            @endforeach
                            ...
                            <div class="po-fg">
                                ...
                                <textarea class="po-textarea" name="purchase_note" rows="3" placeholder="Optional purchase notes…"></textarea>
                            </div>
                        </div>
                ...
                <div class="po-footer">
                    ...
                    <button type="button" id="submitPurchaseBtn" class="po-btn po-btn-primary"
                        :disabled="formSubmitting" @click="submitPurchaseForm">
                        <span id="submitBtnText" v-show="!formSubmitting"><i class="fas fa-save"></i> Submit
                            Purchase</span>
                        <span id="submitBtnSpinner" v-show="formSubmitting" style="display:inline-flex;"><span
                                class="po-spinner"></span> Submitting…</span>
                    </button>
                </div>
            </div>
            ...
            <div v-if="formErrors.length" class="po-card po-card-body"
                ...
                <ul class="mb-0" style="padding-left:1rem;">
                    <li v-for="(msg, idx) in formErrors" :key="idx" style="margin-bottom:4px;">
                        @{{ msg }}</li>
                </ul>
            </div>
        </form>

        <div v-if="warehouseModal.open" class="po-modal-overlay" @click.self="closeWarehouseModal">
            <div class="po-modal" style="max-width: 760px;">
                <div class="po-modal-header">
                    <div class="po-modal-title-block">
                        <p class="po-modal-title">
                            <i class="fas fa-warehouse" style="color:var(--t500);margin-right:6px;"></i>
                            Warehouse Manage
                        </p>
                        <p class="po-modal-subtitle">Create, list, edit and delete warehouses</p>
                    </div>
                    <button type="button" class="po-modal-close" @click="closeWarehouseModal">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
                ...
                <div class="po-modal-body">
                    <div style="display:flex;gap:8px;margin-bottom:14px;flex-wrap:wrap;">
                        <button type="button" class="po-btn po-btn-sm"
                            :class="warehouseModal.mode === 'list' ? 'po-btn-primary' : 'po-btn-ghost'"
                            @click="setWarehouseMode('list')">
                            <i class="fas fa-list"></i> List
                        </button>
                        <button type="button" class="po-btn po-btn-sm"
                            :class="warehouseModal.mode === 'create' ? 'po-btn-primary' : 'po-btn-ghost'"
                            @click="setWarehouseMode('create')">
                            <i class="fas fa-plus"></i> Create
                        </button>
                    </div>

                    <div v-if="warehouseModal.error" style="margin-bottom:12px;color:var(--err);font-weight:700;">
                        @{{ warehouseModal.error }}
                    </div>

                    <div v-if="warehouseModal.mode === 'list'">
                        <div class="po-bc-table-wrap">
                            <table class="po-bc-table">
                                ...
                                <tbody>
                                    <tr v-if="warehouseModal.loading">
                                        <td colspan="5" style="text-align:center;">Loading...</td>
                                    </tr>
                                    <tr v-else-if="!warehouseModal.items.length">
                                        <td colspan="5" style="text-align:center;">No warehouse found.</td>
                                    </tr>
                                    <tr v-for="(warehouse, index) in warehouseModal.items" :key="warehouse.id">
                                        <td>@{{ index + 1 }}</td>
                                        ...
                                        <td>
                                            <button type="button" class="po-btn po-btn-sm po-btn-ghost"
                                                @click="editWarehouse(warehouse)" title="Edit">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <button type="button" class="po-btn po-btn-sm po-btn-outline-danger"
                                                @click="deleteWarehouse(warehouse)" title="Delete">
                    ...
                    <div v-if="warehouseModal.mode === 'create' || warehouseModal.mode === 'edit'">
                        <div class="po-grid-2">
                            <div class="po-fg">
                                <label class="po-label">Title <span class="req">*</span></label>
                                <input type="text" class="po-input" v-model="warehouseModal.form.title"
                                    placeholder="Warehouse title">
                            </div>
                            <div class="po-fg" v-if="warehouseModal.mode === 'edit'">
                                <label class="po-label">Status</label>
                                <select class="po-input po-select" v-model="warehouseModal.form.status">
                                    <option value="active">Active</option>
                                    <option value="inactive">Inactive</option>
                                </select>
                            </div>
                        </div>
                        <div class="po-fg" style="margin-top:10px;">
                            <label class="po-label">Address</label>
                            <textarea class="po-textarea" rows="2" v-model="warehouseModal.form.address"
                                placeholder="Warehouse address"></textarea>
                        </div>
                        <div class="po-fg" style="margin-top:10px;">
                            <label class="po-label">Description</label>
                            <textarea class="po-textarea" rows="3" v-model="warehouseModal.form.description"
                                placeholder="Warehouse description"></textarea>
                        </div>
                    </div>
                </div>

                <div class="po-modal-footer">
                    <button type="button" class="po-btn po-btn-ghost" @click="closeWarehouseModal">
                        Cancel
                    </button>
                    <button v-if="warehouseModal.mode === 'create'" type="button" class="po-btn po-btn-primary"
                        :disabled="warehouseModal.saving" @click="saveWarehouse">
                        <i class="fas fa-save"></i> Save
                    </button>
                    <button v-if="warehouseModal.mode === 'edit'" type="button" class="po-btn po-btn-primary"
                        :disabled="warehouseModal.saving" @click="updateWarehouse">
        ...
        <div v-if="barcodeModal.open" class="po-modal-overlay" @click.self="closeBarcodeModal">
            <div class="po-modal">
                ...
                <div class="po-modal-body" v-if="barcodeModal.item">
                    ...
                    <!-- Variant strip with change option -->
                    <div v-if="barcodeModal.item.variant_combination_id" class="po-modal-variant-strip">
                        <div class="po-modal-variant-pills">
                            <span style="font-size:11px;color:var(--t700);font-weight:700;margin-right:4px;">
                                <i class="fas fa-tag"></i> Variant:
                            </span>
                            <span class="po-modal-variant-pill">@{{ barcodeModal.item.variantLabel }}</span>
                        </div>
                        <button type="button" class="po-modal-change-btn" @click="toggleModalVariantEdit">
                            <i class="fas fa-exchange-alt"></i>
                            @{{ barcodeModal.editingVariant ? 'Cancel' : 'Change' }}
                        </button>
                    </div>

                    <!-- Inline variant re-selector -->
                    <div v-if="barcodeModal.editingVariant && barcodeModal.item" class="po-variant-panel"
                        style="margin-bottom:12px;">
                        <div class="po-variant-title" style="font-size:11px;">
                            <i class="fas fa-th-list"></i> Select New Variant
                        </div>
                        <div class="po-modal-attr-grid">
                            <div v-for="attr in getModalVariantAttributes()" :key="attr" class="po-attr-fg">
                                <label class="po-attr-label">@{{ attr }}</label>
                                <select class="po-attr-select" v-model="barcodeModal.variantSelections[attr]"
                                    @change="onModalVariantChange">
                                    <option value="">— @{{ attr }}</option>
                                    <option v-for="val in barcodeModal.product.product_variants[attr]"
                                        :key="val" :value="val">@{{ val }}</option>
                                </select>
                            </div>
                        </div>
                        <div v-if="barcodeModal.matchedVariant" class="po-comb-strip" style="margin-bottom:8px;">
                            <span v-for="(val, key) in barcodeModal.matchedVariant" :key="key">
                                <span v-if="key !== 'id'" class="po-comb-pill">@{{ key }}:
                                    @{{ val }}</span>
                            </span>
                        </div>
                        <button type="button" class="po-add-btn" style="font-size:12px;padding:6px 14px;"
                            :disabled="!barcodeModal.matchedVariant" @click="applyModalVariantChange">
...
@section('footer_js')
    ...
    @php
        $purchaseWarehousesForJs = $productWarehouses->map(function ($warehouse) {
            return [
                'id' => $warehouse->id,
    ...
    @endphp
    <script>
        window.purchaseWarehouses = @json($purchaseWarehousesForJs);
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (form && submitBtn) {
                ...
                form.addEventListener('invalid', function() {
                    ...
                    submitBtnText.style.display = 'inline-flex';
