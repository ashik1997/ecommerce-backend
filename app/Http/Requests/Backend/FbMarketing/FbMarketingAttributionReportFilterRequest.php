...
class FbMarketingAttributionReportFilterRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        $this->merge([
            ...
            'evidence_state' => $this->filled('evidence_state') ? strtolower(trim((string) $this->input('evidence_state'))) : null,
            'attribution_method' => $this->filled('attribution_method') ? strtolower(trim((string) $this->input('attribution_method'))) : null,
        ]);
    }
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator) {
            ...
            if ($to->gte($from) && $from->diffInDays($to) > 365) {
                $validator->errors()->add('to_date', 'The attribution report range may not exceed 366 calendar days.');
            }
        });
    }
}
