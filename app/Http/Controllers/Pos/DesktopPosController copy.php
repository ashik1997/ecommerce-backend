...
class DesktopPosController extends Controller
{
    ...
    public function productsByCategory(Request $request)
    {
        ...
        $items = $query->select('products.*')
            ...
            ->map(function (Product $product) use ($warehouseId) {
                ...
                // ProductUnitPricing
                $unitPricing = ProductUnitPricing::where('product_id', $product->id)->get();
    ...
    public function productsByBarcode(Request $request)
    {
        ...
        if ($variant) {
            ...
            $productData['stock'] = $this->getVariantStockForWarehouse($variant, $warehouseId, $code);
            ...
            // $productData['stock'] = $this->getProductStockForWarehouse($product, $warehouseId, $code);
    ...
    public function productsByBarcodeOld(Request $request)
    {
        $code = trim($request->get('code', ''));
        $warehouseId = $request->get('warehouse_id');
    ...
    protected function allocatePosSaleUnits(ProductOrder $order, ProductOrderProduct $orderProduct, Product $product, array $item, $warehouseId, $variantId = null): array
    {
        ...
        if ($remaining > 0) {
            $fifoQuery = ProductPurchaseOrderProductUnit::where('product_id', $product->id)
                ->where('unit_status', 'instock')
                ->orderBy('id')
                ->lockForUpdate();

            if ($warehouseId) {
                $fifoQuery->where('product_warehouse_id', $warehouseId);
            }
            if ($variantId) {
                $fifoQuery->where('variant_combination_id', $variantId);
            }
            if ($unitCode) {
                $fifoQuery->where('code', '!=', $unitCode);
            }

            $units = $units->merge($fifoQuery->limit($remaining)->get());
        }
    ...
    protected function decrementPosProductStock(Product $product, $warehouseId, $variantId, float $qty): void
    {
        ...
        if ($variantId) {
            $stockQuery->where('variant_combination_id', $variantId);
        }

        $affected = $stockQuery->decrement('qty', $qty);
        if ($affected < 1) {
            throw new \Exception("Insufficient stock balance for product {$product->id} - {$product->name}.");
        }
    }
    ...
    protected function posCustomerPayload(Customer $customer): array
    {
        ...
        return [
            ...
            'due_amount' => (float) ProductOrder::where('customer_id', $customer->id)->sum('due_amount'),
    ...
    public function createOrderOld(Request $request)
    {
        ...
        if ($customer && $newDueAmount > 0) {
            ...
            $currentDue = (float) ProductOrder::where('customer_id', $customer->id)->sum('due_amount');
        ...
        try {
            ...
            $this->dispatchPosExternalActions(
                $order,
                $deliveryInfo,
                $customer && isset($request->sms_send_to_customer),
                $warehouseId,
                $totalPaid
            );
    ...
    {
        $order->loadMissing(['order_products', 'customer']);
        ...
        return [
            ...
            'totals' => [
                ...
                'extra_charge' => (float) data_get($order->other_charges, 'extra_charge', 0),
                'delivery_charge' => (float) data_get($order->other_charges, 'delivery_charge', 0),
    ...
    protected function getProductStockForWarehouse(Product $product, $warehouseId, $code = null): int
    {
        ...
        $query = ProductPurchaseOrderProductUnit::query()
            ->where('unit_status', 'instock')
            ->where('product_id', $product->id);
        // ->where('status', 'active');
