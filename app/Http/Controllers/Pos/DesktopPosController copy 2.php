...
class DesktopPosController extends Controller
{
        ...
        $items = $query->select('products.*')
            ...
            ->map(function (Product $product) use ($warehouseId) {
                ...
                if ($product->has_variant && $stock_items->count() > 0) {
                    ...
                    // First loop: collect stock data and variant data separately and stock by variant combination key
    ...
    public function print($slug)
    {
        ...
        $qrData = rtrim($website, '/') . '/order-invoice/' . $order->slug;
    ...
    private function calculateOrderTotalsFromCart(array $cart, array $requestTotals): array
    {
        ...
        foreach ($cart as &$item) {
            ...
            $lineTotal = round($finalUnitPrice * $qty);

            $item['final_price'] = $lineTotal;
            $item['discount']['amount'] = round($discountPerUnit * $qty);
        ...
        $extraCharge = (float) data_get($requestTotals, 'extra_charge', 0);
